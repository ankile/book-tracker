...
//                                            migrate-author-ids.ts first)
//   doc updatedAt                         -> never written
//
// Idempotent: a clean re-run writes 0 ops.
//
//   node migrate-person-names.ts                    # emulator dry-run
//   node migrate-person-names.ts --apply            # emulator apply
//   node migrate-person-names.ts --prod             # prod dry-run
//   node migrate-person-names.ts --prod --apply     # prod apply (typed confirm)
import { parseFlags, connect, batcher } from './migrate-lib.ts';
import { AUTHOR_KINDS, splitPersonName } from './src/lib/utils/authors.ts';
...
for (const user of users.docs) {
  ...
  for (const authorDoc of authorDocs.docs) {
    ...
    if (!AUTHOR_KINDS.includes(a.kind)) throw new Error(`author doc without valid kind (run migrate-author-ids.ts first): ${authorDoc.ref.path}`);
