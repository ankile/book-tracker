...
//
//   node db-snapshot.js                # emulator (FIRESTORE_EMULATOR_HOST)
//   node db-snapshot.js --prod         # production (read-only, no confirm)
//   node db-snapshot.js --prod --database=recovered   # a restored backup db
import { mkdirSync, writeFileSync } from 'node:fs';
import { parseFlags, connect, encodeValue, PROJECT_ID } from './migrate-lib.js';
...
async function pooled(fn) {
  while (running >= LIMIT) await new Promise((r) => waiters.push(r));
  running += 1;
  try {
    return await fn();
  } finally {
...
async function walkDocRef(docRef) {
