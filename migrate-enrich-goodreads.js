...
//
//   node migrate-enrich-goodreads.js                    # emulator dry-run
//   node migrate-enrich-goodreads.js --apply            # emulator apply
//   node migrate-enrich-goodreads.js --prod             # prod dry-run
//   node migrate-enrich-goodreads.js --prod --apply     # prod apply (typed confirm)
...
import { goodreadsIsbnUrl, parseGoodreadsHtml } from './src/lib/utils/goodreads.js';
...
async function lookupGoodreads(isbn13) {
...
for (const user of users.docs) {
  ...
  for (const book of books.docs) {
    ...
    const patch = mergeMetadata(b, gr);
