...
| 2026-08-29 | `migrate-purge-deleted-accounts.ts` (not run) | SEC-006 account deletion as a soft delete, code `06207ee`. Deployed 21:1x–21:2xZ with the pinned CLI: `functions:deleteUserDocument,toggl,admin,publicweb` then `firestore:rules`, then a build + `functions:publicweb,hosting` re-pin (Hosting version `1788037887966`). `deleteUserDocument` stamps `deletedAt` on `users/{uid}` (merge, only where absent, in a transaction) and on every `profiles` doc of the uid (paged by document id, since the tombstone does not change the `where uid==` query) and removes nothing; `publicWeb.ts`/`toggl.ts`/`admin.ts`/rules treat the tombstone as absence. Ruleset `b09b4d1d` == source (6fd00992, 65502 B). Sole revisions after GC: `publicweb-00098-ruf`, `toggl-syncqueue-00019-hex` (removed the stale `fh-58209d210cbd59b7` tag with `--to-latest`, deleted `publicweb-00094-ver`/`00097-bec`, `toggl-syncqueue-00018-vub`). No data migration and no prod writes; `migrate-purge-deleted-accounts.ts` was written and rehearsed in the emulator only. Smoke through book.ankile.com: `/profiles/lars` 200 (still in sitemap), unknown 404, `/sitemap.xml` 200, `.json` 200, `/` 200; anon REST create on profiles + profileOwners 403, savetoken 401. No WARNING+ logs after deploy. Tests: rules+emulator 2+94 (new `tests/account-deletion-emulator.test.ts`: compiled trigger over a full account tree, purge rehearsal), functions 129, unit 283, PWA 8, artifacts 4. |
