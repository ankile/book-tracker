...
| 2026-08-29 | `migrate-profile-owners.ts` | Rules-sitting compliance pass (owner request, evening): `db-audit.ts` gained the rules-sitting invariants — `profile.owner-record-missing` / `-mismatch`, `profile.account-missing`, `profile-owner.*`, and `book.rules-shape` / `authordoc.rules-shape` / `profile.rules-shape` through `rules-shape.ts`, a mirror of the rules allowlists and caps that `tests/firestore-rules.test.ts` agreement-tests against the emulator (every fixture judged by both, verdicts must match both ways). Baseline prod audit with the new invariants: zero shape or ownership findings across 221 books / 181 authors / 1 profile / 1 record / 17 users (the owner's `profileOwners` record had already been written by the Me page's stats sync after the follow-up release), so the backfill was NOT needed in prod: `--prod` dry-run printed `ok … already names lars`, 0 writes, and no `--apply` was run. Emulator rehearsal against snapshot `2026-08-29T20-52-54.729Z-prod`: record removed in the emulator → audit shows exactly `profile.owner-record-missing` (+1 finding, profile-owners 0) → apply 1 write → apply 0 writes → post audit identical to the prod baseline. No triggers listen on `profileOwners`/`profiles`, so the Firestore-only emulator was a faithful rehearsal. Script kept for any future profile that predates the record (the rules also self-heal on the profile's next update/delete batch). |
