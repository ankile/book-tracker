...
| 2026-08-30 | `migrate-toggl-tokens.ts` | SEC-004 + SEC-097 (order items 8+9), code `74530eb` + artifacts `c53453a`, deployed 01:32–01:38Z with the pinned CLI (`firestore,functions:toggl,functions:deleteUserDocument,hosting`; Hosting pulled pinned `publicweb` along). New `secrets` Firestore database (eur3, delete protection, NO backup schedule on purpose — a credential is re-issuable) created 01:29Z; deny-all `firestore-secrets.rules` released; both rulesets byte-verified (default `5129076e…` 66712 B, secrets `6301051c…` 677 B). Migration: baseline audit showed `user.toggl-legacy-token` ×1 (the owner, the only credential; timer idle, queue quiet) + the known 10 progress-source baseline rows; snapshot `2026-08-30T01-31-27.279Z-prod.json` (taken BEFORE the move, so it holds the old token — dead after rotation); dry-run then `--prod --apply --rotate`: 1 credential moved to `secrets:togglTokens/{uid}` and rotated at Toggl (token-authenticated `POST /me/reset_token` → fresh 32-char token stored server-side; the old value, including every IndexedDB copy, is invalid at Toggl); post audit findings 11→10, `toggl-secrets: 1`, no orphan/mismatch. IAM (SEC-097): `publicweb-runtime@` viewer re-bound with condition `resource.name.startsWith(…databases/(default))`, unconditional binding removed; proof as the deployed identity (temp tokenCreator grant → probes, status codes only → revoked, SA policy clean): default reads 200, secrets get/list/query 403. GC: `publicweb-00098-ruf`/`toggl-syncqueue-00019-hex` deleted, old fh tag removed, book.ankile.com on `publicweb-00101-civ`. Smoke 200/200/404/200/200 + fresh-origin re-smoke after the IAM window; only WARNING is the probe's own 404 request log. Client: status-only user decoder (legacy shape is a decode error), sign-out terminates the client and clears IndexedDB then lands on `/`. Tests: check clean, unit 283, rules+emulator 2+97, functions 129, PWA 8, artifacts 4; 9 mutants all KILLED (mirror-carries-token, config-from-user-doc, trigger/cleartoken-keep-credential, decoder-accepts-legacy, gate-accepts-legacy, purge-keeps-credential, migration-keeps-legacy-mirror, audit-misses-legacy-token). Repeat-probe commands: `gcloud auth print-access-token --impersonate-service-account=publicweb-runtime@… --project book-tracker-d8f24 --account=lars.ankile@gmail.com` after a temporary `roles/iam.serviceAccountTokenCreator` self-grant on the SA, then Firestore REST GETs with `-o /dev/null -w '%{http_code}'`. |
| 2026-08-30 | (no script — SEC-004/097 review follow-ups) | Three-face review of `74530eb` (worktrees /private/tmp/sec004-{flow,iam,client}; verdicts renamed SECURE / SECURE-WITH-FINDINGS / BROKEN per owner — all three SECURE-WITH-FINDINGS, none BROKEN; the IAM face confirmed the deployed condition live and non-bypassable, and the flow face's top concern — an unverified IAM boundary — was already met by the as-the-identity denial probe in the release row above). Follow-up `8b1d2ac` + artifacts `67c0c6b`, deployed 02:1x–02:2xZ (`functions:toggl,functions:deleteUserDocument,hosting`): sign-out surfaces a refused multi-tab IndexedDB clear (alert instead of a hidden console.warn) and always reloads via finally even when Auth sign-out or the clear throws, and drops `togglStuckReported:*` localStorage keys; the deletion trigger tombstones BEFORE deleting the credential and `savetoken` re-reads the tombstone after writing a credential and deletes it again if the account died mid-call (closing the strand-a-live-credential race; order pinned in the trigger test); an interrupted `--rotate` apply resumes safely — an existing secret is never rotated or overwritten, the re-run only finishes the mirror. 3 fix-mutants all KILLED (re-check off, trigger order swapped, guard off). Not fixed by design: the deploy-window decode break for legacy user docs (migration already run, crash-visible is the house style); secrets readable by Google-managed project agents (inherent, recorded as SEC-097 residual). GC publicweb-00101-civ/toggl-syncqueue-00020-zuj; smoke 200/200/404/200/200; only WARNING is a request log. Tests: check clean, functions 130, rules+emulator 2+98, unit 283, PWA 8, artifacts 4. |
| 2026-08-30 | (no script — SEC-014 monitor phase) | Queue step 12, pulled ahead of SEC-007 CSP (owner decision: close the Highs first; the enforcement flip is the SEC-068 fix). Code `227d118` + artifacts `30a1247` + client pin tests `4495031`, deployed 04:59Z with the pinned CLI (`functions:toggl,functions:telemetry,functions:booksapi,functions:admin,hosting`). Registration done headless: reCAPTCHA Enterprise API enabled, score key `6Ldbm58tAAAAAGj4KhxJlm4yagS848O6dBg47p8_` (public identifier) created via `gcloud recaptcha keys create --web --integration-type=score --project book-tracker-d8f24 --account=lars.ankile@gmail.com` for book.ankile.com + the default domains + localhost, bound to the web app via the firebaseappcheck REST `recaptchaEnterpriseConfig?updateMask=siteKey` PATCH (tokenTtl 3600s). Client initializes App Check in every non-emulator browser context (emulator runs skip it; plain dev sets the debug flag — register its console token before enforcing); all eight callables open with `logAppCheckPresence` (source-pinned). Verified: smoke 200/200/404/200 through book.ankile.com, site key present in the served chunk, anonymous `telemetry-reportissue` POST → 401 with `appcheck.monitor {fn, token: missing}` in Cloud Logging 05:00:27Z; 5 mutants killed (wrong key / gate inverted / debug-flag unconditional / dropped call site / present-missing swap); GC `publicweb-00104-qom` + old fh tag (now `publicweb-00107-yax`, `fh-091446c58d9439bc`). Firestore and the callables stay UNENFORCED until the monitoring window shows genuine clients presenting tokens; the flip is `enforceAppCheck: true` + Firestore ENFORCED and closes SEC-068. |
