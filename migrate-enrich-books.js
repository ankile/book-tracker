...
// src/lib/utils/bookMetadata.js) on every book doc, looking each stored
...
// migrate-normalize-books.js — owns these fields' defaults: if normalize
...
//
//   node migrate-enrich-books.js                    # emulator dry-run
//   node migrate-enrich-books.js --apply            # emulator apply
//   node migrate-enrich-books.js --prod             # prod dry-run
//   node migrate-enrich-books.js --prod --apply     # prod apply (typed confirm)
import { readFileSync, writeFileSync, existsSync } from 'node:fs';
import { parseFlags, connect, batcher } from './migrate-lib.js';
import { normalizeIsbn } from './src/lib/utils/isbn.js';
import { parseOpenLibraryBook, EMPTY_METADATA, METADATA_FIELDS } from './src/lib/utils/bookMetadata.js';
...
for (const user of users.docs) {
  ...
  for (const book of books.docs) {
    ...
    for (const field of METADATA_FIELDS) {
      if (b[field] === undefined) patch[field] = meta ? meta[field] : EMPTY_METADATA[field];
    }
